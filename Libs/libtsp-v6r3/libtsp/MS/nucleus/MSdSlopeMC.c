...
Routine:
  double MSdSlopeMC (int k, const double x[], const double y[], int N)
...
Author / revision:
  ...
  $Revision: 1.4 $  $Date: 2003/11/11 18:46:30 $
...
/* Middle point */
  if (k > 0 && k < N - 1)
    S = MS_slMidMC (x[k] - x[k-1], x[k+1] - x[k],
                    y[k] - y[k-1], y[k+1] - y[k]);

/* End point: beginning */
  else if (k == 0 && N > 2)
    S = MS_slEndMC (x[1] - x[0], x[2] - x[1],
		    y[1] - y[0], y[2] - y[1]);

/* End point: end */
  else if (k == N - 1 && N > 2)
    S = MS_slEndMC (x[N-1] - x[N-2], x[N-2] - x[N-3],
		    y[N-1] - y[N-2], y[N-2] - y[N-3]);
  
/* Special case: linear interpolation */
  else if (N == 2 && (k == 0 || k == 1))
    S = MS_slMidMC (x[1] - x[0], x[1] - x[0],
		    y[1] - y[0], y[1] - y[0]);
