...
Author / revision:
  ...
  $Revision: 1.23 $  $Date: 2003/11/11 18:45:47 $
...
/* Check the reference abscissa values */
  for (k = 1; k < N; ++k) {
    if (x[k] < x[k-1])
      UThalt ("MSfIntMC: %s", MSM_NonIncrAbs);
  }
  ...
  if (N == 1) {
    for (i = 0; i < Ni; ++i) {
      if (xi[i] != x[0])
	UThalt ("MSfIntMC: %s: \"%g\"", MSM_BadAbsVal, xi[i]);
      yi[i] = y[0];
    }
  }
...
*/
	...
	dh = MSfSlopeMC (k, x, y, N);
      }
      else if (kp == k + 1) {
	dh = dl;
	dl = MSfSlopeMC (k - 1, x, y, N);
      }
      else if (k != kp) {
	dl = MSfSlopeMC (k - 1, x, y, N);
	dh = MSfSlopeMC (k, x, y, N);
      }
      yi[i] = (float) MSevalMC (xi[i], x[k-1], x[k], y[k-1], y[k], dl, dh);
      kp = k;
    }
  }
  return;
