...
Routine:
  void MAlPrint (FILE *fp, const char Header[], const long int *A[], int Nrow,
                 int Ncol)

Purpose:
  Print a long integer matrix along with a header text

Description:
  This routine prints the values of a long int matrix along with an identifying
...
Parameters:
   ...
   -> const long int *A[]
      ...
      to have the const attribute, an explicit cast to (const long int **) is
      required.
   -> int Nrow
...
Author / revision:
  ...
  $Revision: 1.6 $  $Date: 2003/05/09 01:46:06 $
