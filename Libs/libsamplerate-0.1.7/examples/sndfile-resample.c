...
// hacked by stp to overwrite the input file
// use like
//		for i in *; do sndfile-resample -c 0 -to 44100 -m $i $i.ss; done
//		for k in *; do pushd $k; for i in *; do sndfile-resample -c 0 -to 44100 -m $i $i.ss; done; popd; done
//
...
#define DEBUG_P	if (verbose) printf		// turn debugging printfs on/off
...
{	SNDFILE	*infile = 0, *outfile = 0;
	...
	for (k = 1 ; k < argc - 2 ; k++) {
		if (strcmp (argv [k], "--max-speed") == 0)			// max-speed
			max_speed = SF_TRUE ;
		else if (strcmp (argv [k], "-to") == 0)				// target Fs
		{	k ++ ;
			new_sample_rate = atoi (argv [k]) ;
		} else if (strcmp (argv [k], "-by") == 0)			// ratio
		{	k ++ ;
			src_ratio = atof (argv [k]) ;
		} else if (strcmp (argv [k], "-q") == 0)			// quiet
		{	verbose = 0;
		} else if (strcmp (argv [k], "-m") == 0)			// mono
		{	mono = 1;
		} else if (strcmp (argv [k], "-c") == 0)			// convertor sel
		{	k ++ ;
	...
	if (new_sample_rate <= 0 && src_ratio <= 0.0) {			// check rate
		printf ("\nError : bad rate or ratio.\n") ;
		usage_exit (argv [0]) ;
	}
	if (src_get_name (converter) == NULL){					// check conv
		printf ("\nError : bad converter number.\n") ;
		usage_exit (argv [0]) ;
	}
	char * inName = 0;
	char * outName = 0;
	...
	if (argc == 6) {						// cmd = sndfile-resample -c 0 -to 44100 file.wav		-- stp changes
		inName = argv[argc - 1];
		...
		char * dot = rindex(newName, '.');
		strcpy(ext, dot);
		sprintf(dot, "b%s", ext);
//		printf ("Write to open output file '%s'\n", newName) ;
		outName = newName;
		verbose = 0;
	} else {
		inName  = argv[argc - 2];
	...
	printf  ("\tInput File    : %s", inName) ;
	DEBUG_P ("\n\tSample Rate   : %d\n", sfinfo.samplerate) ;
	DEBUG_P ("\tNum Chans     : %d\n", sfinfo.channels) ;
	DEBUG_P ("\tSF Format     : %s%s%s\n", (((sfinfo.format & SF_FORMAT_TYPEMASK) == SF_FORMAT_WAV) ? "Wav:" : ""),
											(((sfinfo.format & SF_FORMAT_SUBMASK) == SF_FORMAT_PCM_16) ? "Pcm16:" : ""),
											(((sfinfo.format & SF_FORMAT_ENDMASK) >> 28) ? "" : "DefEnd")) ;
	DEBUG_P ("\tInput Frames  : %ld\n", (long) sfinfo.frames) ;
	DEBUG_P ("\tDuration      : %5.3f\n", (float) sfinfo.frames / sfinfo.samplerate) ;

	if (new_sample_rate > 0)
	{	src_ratio = (1.0 * new_sample_rate) / sfinfo.samplerate ;
		sfinfo.samplerate = new_sample_rate ;									// set rate
	}
	else if (src_is_valid_ratio (src_ratio))
		sfinfo.samplerate = (int) floor (sfinfo.samplerate * src_ratio) ;		// set rate
	else
	{	printf ("\nNot able to determine new sample rate. Exiting.\n") ;
		sf_close (infile) ;
		exit (1) ;
	}

	if (fabs (src_ratio - 1.0) < 1e-2)
	{	printf ("\t\tIdentical samplerates\n") ;
		sf_close (infile) ;
		exit (0) ;
	}

	if (src_is_valid_ratio (src_ratio) == 0)
	{	printf ("\nError : Sample rate change out of valid range.\n") ;
		sf_close (infile) ;
		exit (1) ;
	}
	if (mono)
	...
	DEBUG_P ("\tOutput file   : %s\n", outName) ;
	DEBUG_P ("\tSample Rate   : %d\n", sfinfo.samplerate) ;

	do																// do SRC
		count = sample_rate_convert (infile, outfile, converter, src_ratio, sfinfo.channels, &gain) ;
	while (count < 0) ;

	DEBUG_P ("\tOutput Frames : %ld\n\n", (long) count) ;

	sf_close (infile) ;
	sf_close (outfile) ;

	char cmd [512];			// rename files
	sprintf(cmd, "mv \"%s\" \"%s\"", outName, inName);
	system(cmd);
