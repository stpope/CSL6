...
public:
    CSLWindow() : DocumentWindow (String("CSL Granulator 1.0"), Colours::lightgrey, DocumentWindow::allButtons, true) {
        setContentComponent (createCSLComponent());	// create the actual component here
		setResizable (true, false);
        setVisible (true);
        centreWithSize (408, 482);					// top window size 8 @ 36 larger than the component
		setUsingNativeTitleBar(true);
// If using the larger analog meters -- do this
//        centreWithSize (408, 636);				// top window size 8 @ 36 larger than the component
    }
